...
# Configure Okta
# vault write -namespace=dev auth/okta/config org_name="$okta_org"
# curl -H "X-Vault-Token: $VAULT_TOKEN" -H "X-Vault-Namespace: $VAULT_NAMESPACE" -X POST $VAULT_ADDR/v1/auth/okta/config -d '{"org_name":"$okta_org"}'
